...
float bug1_obstacle1Driver(bool left){
    // Obstacle Definition
    ...
    Bug1Trajectory = r.Bug1MainLoop(r.pos, goalPos, Union_obstacle,left);
...
float bug1_obstacle2Driver(bool left){
    // Obstacle Definition
...
int main(){
    ...
    float i = bug1_obstacle1Driver(turn);

    float j = bug1_obstacle2Driver(turn);
...
std::vector<std::pair<float,float>> robot::Bug1MainLoop(std::pair<float,float> iniPos, std::pair<float,float> goalPos, std::vector<obstacle> obs, bool turn){
